...
def checkOTP():
    if code == codeEntry.get():
        accept = Label(otp, text='Successful Verification!',
                       fg='green', font=('Arial', 20))
        accept.place(x=230, y=350)
    else:
        refuse = Label(otp, text='Unsuccessful Verification!',
...
mailMsg = Label(otp, text="E-Mail", justify=LEFT,
                bg='#FFF1DC', font=("Arial", 16))
mailMsg.place(x=15, y=40)

receiverMail = Entry(otp, text='mail.gmail.com', width=35,
                     font=("Arial", 20), borderwidth=0)
receiverMail.place(x=100, y=40)
receiverM = StringVar()

sendOTP = Button(otp, text="send OTP", width=8, height=1, font=(
    "Arial", 20), borderwidth=0, bg="#AA5656", fg="black", command=connectingSender)
